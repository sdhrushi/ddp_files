...
# Class for generating masks for image patches
class MaskGenerator:
...
# Class for data augmentation with weak and strong transforms
...
# Custom dataset class for loading images and labels from file lists
...
def build_dataset(is_train, args, train_config=None):
    ...
    assert args.dataset in catalog.keys(), f"Dataset {args.dataset} is not implemented"
    ...
    if entry['type'] == 'special':
        ...
        image_file = os.path.join(entry['path'], f"{path}_images.npy")
        label_file = os.path.join(entry['path'], f"{path}_labels.npy")
        target_transform = None
        dataset = FileListDataset(image_file, label_file, transform, target_transform)    

    return dataset

# Function to build the transform
